/* ***************************************************************************** */
/*    MIKA's ten key typing trainer  Javascript source code                      */
/*                                                    Ver2.05.01 2024/11/29      */
...
MIKA_mes0="●●●  Mika's Ten Key Trainer Ver2.05.01  ●●●";
MIKA_mesta="●● Mika's Ten Key Trainer (%s) ●●";
MIKA_mesi1="Press TAB key to try again";
...
MIKA_key_type_mes="．．";
MIKA_kugiri_mes="Press Enter key to separate Number";
...
// flag=0 表示 flag=1 消去 
	{
		...
		cslput(g,20*16,12*8,MIKA_keymes3); /*「この次は、スペースキーを押してキーガイドの表示を消して練習してみましょうね」 のメッセージを表示 */
		else /* キーガイドメッセージ表示フラグが 2の場合 */
		cslput(g,20*16,12*8,MIKA_keymes4); /*「この次は、スペースキーを押してキーガイドを表示して練習してみましょうね」 のメッセージを表示 */
	}
...
function dispmaxspeedrecord(g,i1,j1,i2,j2) /* ランダム練習 の 最高入力速度と 達成日を表示 */
	{
			...
			a="Maximum Typing Speed"+formatf1(MIKA_type_speed_record[MIKA_type_kind_no],6)+"/min"; /* 最高速度メッセージ作成 */
...
function disptrain(g,mest) /* ランダム練習 実行画面の表示 */
{
		...
		{
			dispmaxspeedrecord(g,3,22,3,53);
		}
...
// flag=0 表示 flag=1 消去 
	{
		...
		cslput(g,5*16,1+offset*8,a); /* 文字列の表示あるいは消去 */
	}
